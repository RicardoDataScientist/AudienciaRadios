...
from statsmodels.tsa.seasonal import seasonal_decompose # <-- NOSSA NOVA IMPORTAÇÃO MÁGICA! ✨
import traceback
import gradio as gr
...
def treinar_e_avaliar_cv(df_limpo, features_finais, target, teste_size, model_params, progress):
    ...
    sns.lineplot(data=df_resultados, ax=ax_pred, palette={'Real': '#0072B2', 'Previsto': '#ff0051'})
    ...
    ax_imp.set(title=f'Importância das Features para {target.upper()} (Top 20)', xlabel='Importância', ylabel='Features')
...
def gerar_grafico_decomposicao(df_historico, target):
    ...
    plt.close('all')
    try:
        # Usamos 'additive' como modelo padrão. Poderia ser 'multiplicative' se a sazonalidade cresce com a tendência.
        ...
        # Se der erro (ex: série muito curta), cria um gráfico de aviso
...
def gerar_grafico_sazonalidade_anual(df_historico, target):
    ...
    plt.close('all')
    try:
        # Garante que a série tenha pelo menos 2 ciclos completos para a decomposição
        ...
        # Calcula a média do efeito sazonal para cada mês
        ...
        ax.axhline(0, color='gray', linestyle='--', linewidth=1) # Linha de base
...
def gerar_graficos_previsao_futura(df_historico_limpo, df_previsoes_futuras, target, df_historico_completo):
    ...
    # Gráfico 1: Previsão Futura vs. Histórico (Últimos 12 meses)
...
def salvar_resultados_zip(target, df_metricas, features_finais, df_resultados_cv, df_previsoes_futuras, figs_dict):
    ...
    # Usar tempfile para garantir um nome de arquivo único e evitar colisões
...
def executar_pipeline_auto(arquivo, coluna_data_orig, target_orig, colunas_features_orig, colunas_fixas_orig, temporal_features_orig, tamanho_previsao_meses, lags, n_estimators, learning_rate, max_depth, early_stopping_rounds, progress=gr.Progress(track_tqdm=True)):
    # --- CORREÇÃO: Validação de Inputs Essenciais ---
    # Adicionamos essa verificação para garantir que o usuário selecionou as colunas antes de rodar.
    ...
    try:
        ...
        while True:
            ...
            yield log_text, None, None, None, None, None, None, gr.update(value=None, visible=False), "### Seleção de Features\n*Aguardando início do processo...*", gr.update(visible=False), "", features_atuais, None, None, None, None, None
            ...
            yield log_text, None, None, None, None, None, None, gr.update(value=None, visible=False), "### Seleção de Features\n*Aguardando início do processo...*", gr.update(visible=False), "", features_atuais + [melhor_nova_feature], None, None, None, None, None
            ...
            yield log_text, None, None, None, None, None, None, gr.update(value=None, visible=False), "### Seleção de Features\n*Aguardando início do processo...*", gr.update(visible=False), "", features_para_refinar, None, None, None, None, None
        ...
        yield log_text, df_res, fig_pred, df_met, fig_imp, fig_shap_sum, fig_shap_force, zip_file, features_md_final, gr.update(visible=True), features_copy_str, features_finais, fig_fut, fig_fut_media, fig_real_media, fig_decomp, fig_sazonal

    except Exception as e:
        log_text += f"\n❌ Ocorreu um erro: {e}\n\n--- Detalhes ---\n{traceback.format_exc()}"
        yield log_text, None, None, None, None, None, None, gr.update(value=None, visible=False), "### Erro\n*Ocorreu um erro durante a execução.*", gr.update(visible=False), "", [], None, None, None, None, None
...
def executar_pipeline_manual(arquivo, coluna_data_orig, target_orig, tamanho_previsao_meses, temporal_features_orig, features_finais_selecionadas, colunas_configuradas_orig, n_estimators, learning_rate, max_depth, early_stopping_rounds, *lista_de_lags, progress=gr.Progress(track_tqdm=True)):
    ...
    log_text = ""
    blank_outputs_for_yield = [None] * 13
    try:
        progress(0, desc="Carregando dados...")
        ...
        yield log_text, df_res, fig_pred, df_met, fig_imp, fig_shap_sum, fig_shap_force, zip_file, features_selecionadas_md, fig_fut, fig_fut_media, fig_real_media, fig_decomp, fig_sazonal
...
def processar_arquivo(arquivo):
    ...
    try:
        ...
        try:
            # Tenta encontrar a coluna de data de forma mais inteligente
...
def reset_all():
    ...
    return [
        ...
        "", None, "", gr.update(visible=False), "", gr.update(value=None, visible=False),
        None, None, None, None, None, None, None, None, None, None,
    ] + lag_ui_updates

# --- CONSTRUÇÃO DA INTERFACE ---
with gr.Blocks(theme=gr.themes.Soft(), title="AutoML de Séries Temporais Pro", css=custom_css) as demo:
    gr.Markdown("# 🤖 AutoML para Séries Temporais com Feature Selection Pro v4.5")
